tool_list_files() {
    ...
    echo -e "${TS} \033[0;36m[Tool Action ($CURRENT_TURN/$MAX_TURNS)] Listing: $FC_PATH\033[0m"
...
tool_get_file_info() {
    ...
    local TS=$(get_log_timestamp)
    echo -e "${TS} \033[0;36m[Tool Action ($CURRENT_TURN/$MAX_TURNS)] Getting Info: $FC_PATH\033[0m"
...
tool_search_files() {
    ...
    local TS=$(get_log_timestamp)
    echo -e "${TS} \033[0;36m[Tool Action ($CURRENT_TURN/$MAX_TURNS)] Searching for \"$FC_QUERY\" in: $FC_PATH\033[0m"
...
tool_grep_definitions() {
    ...
    local TS=$(get_log_timestamp)
    echo -e "${TS} \033[0;36m[Tool Action ($CURRENT_TURN/$MAX_TURNS)] Grep Definitions in: $FC_PATH\033[0m"
...
tool_find_file() {
    ...
    local TS=$(get_log_timestamp)
    echo -e "${TS} \033[0;36m[Tool Action ($CURRENT_TURN/$MAX_TURNS)] Find: $FC_PATTERN in $FC_PATH\033[0m"
...
tool_get_tree() {
    ...
    local TS=$(get_log_timestamp)
    echo -e "${TS} \033[0;36m[Tool Action ($CURRENT_TURN/$MAX_TURNS)] Generating Tree: $FC_PATH (Depth: $FC_DEPTH)\033[0m"
